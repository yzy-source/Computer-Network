...
BEGIN_MESSAGE_MAP(CClientDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_CLOSE, &CClientDlg::OnClickedClose)
...
BOOL CClientDlg::OnInitDialog()
{
	...
	BOOL bFlag=client.Create(2001,SOCK_STREAM,FD_READ|FD_CONNECT|FD_WRITE);//以流方式创建socket
	if(!bFlag)
	{
		MessageBoxW(L"Socket Create Error!");	 		//创建套接口错误处理
		exit(0);
	}
...
void CClientDlg::OnBnClickedOk()
{
	...
	client.Send(m_request.GetBuffer(),(m_request.GetLength()+1)*sizeof(TCHAR),0);      //发送
...
void CClientDlg::OnClickedClose()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(true);
	client.Close();                //关闭连接
